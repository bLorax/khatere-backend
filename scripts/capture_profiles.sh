...
sleep 3
docker compose exec "$CONTAINER" wget -qO /app/cpu.prof "http://127.0.0.1:6060/debug/pprof/profile?seconds=25"
docker compose exec "$CONTAINER" wget -qO /app/heap.prof "http://127.0.0.1:6060/debug/pprof/heap"
docker compose exec "$CONTAINER" wget -qO /app/goroutine.prof "http://127.0.0.1:6060/debug/pprof/goroutine"

wait "$LOAD_PID"

docker cp "$CONTAINER:/app/cpu.prof" "$OUTDIR/cpu.prof"
docker cp "$CONTAINER:/app/heap.prof" "$OUTDIR/heap.prof"
docker cp "$CONTAINER:/app/goroutine.prof" "$OUTDIR/goroutine.prof"
